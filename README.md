...
tests/                    297 tests
outputs/                  committed, and regenerated by `make pipeline`
```
...
The tables and the write-up regenerate byte for byte on any machine, and CI checks the committed copies against a fresh run on Linux.
That took CSVs written with LF endings whatever the platform, and floats at a fixed precision.

The figure is a weaker guarantee, which is worth stating rather than glossing.
Matplotlib rasterises text through freetype, and freetype's version differs between machines, so the PNG is byte-identical across repeated runs on one machine but not across platforms.
CI therefore checks it by running the pipeline twice and comparing, which is what would catch an embedded timestamp or unseeded jitter; a fixed seed for the point jitter and a suppressed PNG text chunk are what make that hold.
